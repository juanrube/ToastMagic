...
class ToastMagic
{
    ...
    {
        $stylePath = 'packages/juanrube/laravel-toast-magic-8/css/laravel-toast-magic-8.css';
        if (File::exists(public_path($stylePath))) {
            return '<link rel="stylesheet" href="' . $this->getDynamicAsset($stylePath) . '">';
        }
        return '<link rel="stylesheet" href="' . url('vendor/juanrube/laravel-toast-magic-8/assets/css/laravel-toast-magic-8.css') . '">';
    }
    ...
    public function scriptsPath(): string
    {
        ...
        if (!empty($config['livewire_enabled'])) {
            $file1 = 'packages/juanrube/laravel-toast-magic-8/js/livewire-v3/laravel-toast-magic-8.js';
            $file2 = 'packages/juanrube/laravel-toast-magic-8/js/livewire-v3/livewire-toast-magic-v3.js';
        ...
        $defaultJsPath = 'packages/juanrube/laravel-toast-magic-8/js/laravel-toast-magic-8.js';
        if (File::exists(public_path($defaultJsPath))) {
            return $this->scriptTag($defaultJsPath);
        }

        return $this->scriptTag('vendor/juanrube/laravel-toast-magic-8/assets/js/laravel-toast-magic-8.js');
    }
    ...
    {
        if (config('laravel-toast-magic-8.system_processing_directory') == 'public') {
    ...
    public function scripts(): string
    {
        ...
        $config = (array)$this->config->get('laravel-toast-magic-8.options');
     ...
     * @throws Exception
     */
    public function add(string $type, string $message, $description = null, array $options = []): void
    {
     ...
     * @throws Exception
     */
    public function info(string $message, $description = null, array $options = []): void
    {
     ...
     * @throws Exception
     */
    public function success(string $message, $description = null, array $options = []): void
    {
        if ($message instanceof MessageBag) {
            $messageString = "";
            foreach ($message->getMessages() as $messageArray) {
     ...
     * @throws Exception
     */
    public function warning(string $message, $description = null, array $options = []): void
    {
        if ($message instanceof MessageBag) {
            $messageString = "";
            foreach ($message->getMessages() as $messageArray) {
     ...
     * @throws Exception
     */
    public function error(string $message, $description = null, array $options = []): void
    {
        if ($message instanceof MessageBag) {
            $messageString = "";
            foreach ($message->getMessages() as $messageArray) {
